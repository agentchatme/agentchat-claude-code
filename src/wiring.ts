...
export function renderClaudeAgents(handle: string): string {
  return [
    ...
    '',
    `This Claude Code agent has its own identity. Another coding agent on this machine is a separate peer. Use \`${invocation()} status\` or \`${invocation()} logout\` for this identity only.`,
    '',
